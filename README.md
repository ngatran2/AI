...
Thực hiện: Đảm bảo file audited hợp lệ.
...
Agent phân tích Requirement và Yêu cầu User cung cấp thông số tải (VUs, Duration, KPIs).
Nếu User không có thông số cụ thể, Agent sẽ Tự động Suggest cấu hình tối ưu dựa trên môi trường (Staging/Prod).
User xác nhận đề xuất -> Agent mới bắt đầu sinh Plan và Script.
Cơ chế đặc biệt (Advanced Features):
