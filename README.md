...
- **Kết quả:** Báo cáo kết quả kiểm thử (Pass/Fail) cập nhật trực tiếp vào file Excel (kế thừa hậu tố `-hl` hoặc `-det`) và lưu tại thư mục `execution/[UC-ID]/reports/`.
...
- **Ra lệnh:** Gõ lệnh `/performance-testing` (Hoặc chat *"Viết performance test plan cho chức năng [UC-ID] với 1000 users"*).
- **Cơ chế đặc biệt (Advanced Features):**
...
- **Kết quả:** Sinh ra một bản Kế hoạch (Performance Test Plan) đầy đủ KPIs, kèm theo file script/data lưu tại `execution/report/` và `execution/[UC-ID]/scripts/`.
